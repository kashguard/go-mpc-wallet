...
- **实施方法**：遵循《docs/mpc-implementation-methodology.md》中的分阶段与质量控制流程
...
下方是基于《MPC 基础设施系统详细设计文档》重新制定的、可直接执行的 Phase 1 计划。相较于 v1.0 版本，本计划将每个阶段进一步拆解为 2-5 天可交付的具体任务，并明确了责任角色、输入产出、验收标准与风险点。
...
### Phase 1A: 协议引擎与DKG落地（W1-W4）

**目标**：在4周内完成 GG18/GG20 协议、DKG 流程与协议状态机的高质量实现，为后续所有上层服务提供可靠的密码学基础。

| 周次 | 任务包 | 责任角色 | 产出 | 验收标准 | 风险 & 缓解 |
| --- | --- | --- | --- | --- | --- |
...
**交付物清单**：
- `internal/mpc/protocol/gg18.go` / `gg20.go` / DKG & 签名实现
- `internal/mpc/session/store.go` 及 Redis/PG schema
- Benchmark + Profiling 报告
- 协议状态机时序图（补充到 detailed-design 文档）

---

### Phase 1B: 上层服务与业务 API（W5-W7）

**目标**：在3周内将密钥、签名、会话等业务逻辑与协议层全面打通，并完成 REST/gRPC API 的对外输出。

| 周次 | 任务包 | 责任角色 | 产出 | 验收标准 | 风险 & 缓解 |
| --- | --- | --- | --- | --- | --- |
| W5 | 阈值签名服务（单次/批量） | 服务负责人 + 协议负责人 | `signing/service.go`、批量签名优化 | 并发 50 TPS、错误率 < 1% | 节点负载热点 → 引入节点打分 + 轮询调度 |
| W6 | 密钥生命周期（轮换/恢复/审计） | 密钥负责人 + 安全工程师 | `key/rotation.go`、审计日志方案 | 支持 Planned Rotation，审计可追溯 | 轮换期间一致性 → 灰度策略 + 双写验证 |
| W7 | API Handler & SDK 对接 | API 团队 + SDK 团队 | 全部 Swagger Handler、Go SDK Demo | 端到端调用成功率 100% | Handler 参数验证遗漏 → 使用 swagger 生成的 Validate |

**交付物清单**：
- 已接入协议层的 `signing`, `key`, `session` 服务
- REST/gRPC API + Go SDK 示例
- 密钥审计与轮换策略文档
- 性能基线报告（50TPS/500ms）

---

### Phase 1C: 系统级测试与MVP交付（W8-W9）

**目标**：完成端到端测试、性能调优、故障注入以及MVP交付前的全量文档/监控完善。

| 周次 | 任务包 | 责任角色 | 产出 | 验收标准 | 风险 & 缓解 |
| --- | --- | --- | --- | --- | --- |
| W8 | 端到端集成测试 + Chaos | 测试负责人 + DevOps | E2E 测试套件、Chaos 实验报告 | 所有关键用例通过，故障恢复 < 60s | 测试环境不足 → 使用 docker-compose+k3d 快速构建 |
| W9 | 性能优化 + 文档交付 | 性能工程师 + 文档负责人 | Profiling 结果、MVP文档包 | 签名 p95 < 500ms、吞吐 ≥ 100 TPS | 性能不达标 → 优化协议参数、并行批次处理 |

**最终交付**：
- MVP Go 服务 & Swagger 文档
- Go SDK + Demo CLI
- 运维文档（部署、监控、告警）
- 测试报告（单元/集成/性能/安全）
